...
// ─── Source 7: Yahoo Finance (free, no API key, raw fetch) ────────────
...
export async function researchYahoo(companyName) {
  try {
    ...
    if (f.totalRevenue?.fmt) parts.push(`Revenue: ${f.totalRevenue.fmt}.`);
    ...
    if (d.fiftyTwoWeekHigh?.fmt && d.fiftyTwoWeekLow?.fmt) {
      parts.push(`52-Week: ${d.fiftyTwoWeekLow.fmt} - ${d.fiftyTwoWeekHigh.fmt}.`);
    }

    if (parts.length === 0) return null;
    return { text: parts.join(" "), source: "Yahoo Finance", url: `https://finance.yahoo.com/quote/${symbol}` };
  } catch (e) {
