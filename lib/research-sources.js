...
export async function researchMetaTags(companyName) {
  try {
    const domain = companyName.toLowerCase().replace(/[^a-z0-9-]/g, "").substring(0, 63);
    if (!domain || domain.length < 2) return null;

    const blocked = ["localhost", "127.0.0.1", "0.0.0.0", "internal", "metadata"];
    if (blocked.includes(domain) || /^\d+$/.test(domain)) return null;
    for (const tld of COMMON_DOMAIN_TRIES) {
      try {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 5000);
        const res = await fetchWithTimeout(`https://${domain}.${tld}`, {
...
export async function researchCrunchbase(companyName) {
  ...
  try {
    const res = await fetchWithTimeout("https://api.crunchbase.com/api/v4/searches/organizations", {
      method: "POST",
      headers: {
...
// ─── Source 7: Yahoo Finance (free, no API key needed) ──────────────────

export async function researchYahoo(companyName) {
  try {
    const yahooFinance = await import("yahoo-finance2");
    const { quotes } = await yahooFinance.default.search(companyName, {}, { validateResult: false });
    if (!quotes || quotes.length === 0) return null;

    const symbol = quotes[0].symbol;
    const modules = ["assetProfile", "summaryDetail", "defaultKeyStatistics", "financialData"];
    const summary = await yahooFinance.default.quoteSummary(symbol, { modules }, { validateResult: false });
    ...
    const parts = [];
    if (a.longBusinessSummary) parts.push(a.longBusinessSummary);
    if (a.sector) parts.push(`Sector: ${a.sector}. Industry: ${a.industry || "N/A"}.`);
    if (f.currentPrice?.fmt) parts.push(`Price: ${f.currentPrice.fmt}.`);
    if (k.marketCap?.fmt) parts.push(`Market Cap: ${k.marketCap.fmt}.`);
    if (f.totalRevenue?.fmt) parts.push(`Revenue (TTM): ${f.totalRevenue.fmt}.`);
    if (d.trailingPE?.fmt) parts.push(`P/E Ratio: ${d.trailingPE.fmt}.`);
    if (d.dividendYield?.fmt) parts.push(`Dividend Yield: ${d.dividendYield.fmt}.`);
    if (d.fiftyTwoWeekHigh?.fmt && d.fiftyTwoWeekLow?.fmt) {
      parts.push(`52-Week Range: ${d.fiftyTwoWeekLow.fmt} - ${d.fiftyTwoWeekHigh.fmt}.`);
    }
    if (d.beta?.fmt) parts.push(`Beta: ${d.beta.fmt}.`);
