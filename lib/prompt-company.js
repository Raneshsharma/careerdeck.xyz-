...
## YOUR MISSION
Generate a Company Dossier with exactly 12 sections (plus Bonus). For each section, write one cohesive, 3-5 sentence paragraph that synthesises the provided data to answer the embedded guiding questions. Do NOT answer in a Q&A or bullet format — the paragraph must flow naturally, but ensure every question is addressed.

## RULES (IRONCLAD)
1. **Only use provided data.** If a question cannot be answered from [COMPANY_RESEARCH] or [NEWS_ARTICLES], state: "Information not available from current research." Never invent facts.
...
3. **Adhere strictly to the section headers.** Output them exactly as shown, in order.
4. **Paragraph length:** 3-5 sentences per section, except #11 and #12 which may extend to 6-8 if rich data exists.
5. **Inference marking:** If a conclusion is strongly implied but not explicitly stated, prefix with "(Likely)".
6. **End with the Bonus 5 Highest-Value Questions** as a numbered list.

## SECTION GUIDELINES
(Answer these guiding questions within each paragraph. Weave into flowing prose.)
...
## Bonus: The 5 Highest-Value Questions
1. (question that connects money, priorities, and role)
...
These must be directly derivable from the dossier and framed as questions the candidate should know cold.
